...
class LeverTile extends Lever implements RedstonePowerSource
{
    ...
    public function use(Player $player, int $x, int $y, int $z): bool
    {
        ...
        else if ($rot == 5 || $rot == 6) $player->getLevel()->scheduleNeighbourBlockUpdates(new Vector3($x, $y - 1, $z));
        else if ($rot == 7 || $rot == 8) $player->getLevel()->scheduleNeighbourBlockUpdates(new Vector3($x, $y + 1, $z));
        return true;
    }
